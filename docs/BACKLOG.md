...
| E3 | **`SerdeFactory` for Schema Registry, in a real `libs/serde-confluent`** | The registry client, its caches and its credential config are needed identically by browse, produce, resend, tracking and the future SR service. `build.mill` and `ARCHITECTURE.md` already describe this module as existing; it does not. | The SPI seam is built and correct: `ClusterSerdes.SerdeFactory` returns `Resource[F, Either[String, Serde[F]]]` precisely so a registry that is down yields a disabled picker row rather than a crash. Nothing implements it. | The module, an HTTP client over `libs/http`'s `UpstreamClient`/`Failover`, the two caches, and Avro / JSON Schema / Protobuf serdes. | L |
...
| E5 | **`Main` + client module + image for `topic`, `message` and `consumer`** | Without these three the product is not the distributed system the README describes, the fault-isolation e2e suite can only ever cover one of four services, and `deployment/compose` cannot demonstrate the product. Every operations and testing item downstream depends on them. | Only `gateway` and `cluster` have a `Main`; only `cluster` has an HTTP client module. All three services' wiring exists and runs inside the all-in-one. | Three `Main`s, two client modules, three `KuiImage`s, and the compose topology. | M |
| E6 | **`LogDirDto` carries per-topic-partition entries** (TD-017) | Four screens are waiting on the same field: topic detail size, the partition table's size column, the broker log-dir tab and PA-003. | Directory-level log dirs work end to end. | One contract field and its adapter mapping. Then the four consumers, and `VirtualizedTable`'s first real caller (TD-018). | S |
| E7 | **CI actually runs the test suites** | Every wave below adds tests that will not be run. This is the cheapest item in the document and it protects all the others. | A well-built workflow whose `test` job runs three commands over two of 47 test source trees. | Add the JVM suites, each Scala.js module, `openApiCheck` (4 modules) and `docs.errorCodes --check`. Fix `deployment/compose/smoke.sh` and the four artifacts that still assert `GET /api/v1/ping`, an endpoint deleted in M1. | S |
...
| --- | --- | --- | --- |
| D1 | **CI runs 2 of 47 test source trees** (E7) | Every wave adds suites nothing will run. | S |
...
| D8 | **`deployment/compose/smoke.sh` fails on a clean run**, and four shipped artifacts still document `GET /api/v1/ping`, deleted in M1 | It is the command the root README gives a new contributor. | S |
| D9 | **The README's "one process or eleven" is false** (E5), and its "What CI runs" table is wrong in two rows | Both are the first things a prospective contributor reads. | S |
