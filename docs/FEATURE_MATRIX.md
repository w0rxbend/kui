...
> Laminar browser build with TypeScript, SolidJS and Vite. Rows whose Evidence column cites a
> `.scala` path under `frontend/`, or a Scala.js mechanism, were verified against an implementation
> that no longer exists. The *feature* was ported; the *verification* was not re-run. Those rows are
> marked inline where they occur, and re-verifying them against `frontend/packages/` is outstanding
> work. Backend evidence is unaffected.
...
The wave-4 pass of 2026-09-06 moved six rows and, for the first time since this paragraph was
made checkable, it moved one **down**. Up: `SR-002` (registering a schema, the one M6 bullet with
no endpoint behind it, now an endpoint and a dialogue and a browser case), `SR-001` (its two named
gaps — nothing sorts, no master–detail — both closed), `MT-002` (a Prometheus exposition is read,
configured in the quickstart, and measured answering 288 buckets of which 13 carry a rate) and
`KU-004` (lazy feature loading, on a Vite manifest **and** the guard that reads it). Down:
`CG-005`, which said a control was on the group detail page and was verified saying so in September
against a screen ADR-048 then deleted; the port did not carry the control and nothing in this
repository called its client function, so the row was moved to `IMPLEMENTING`. It is `COMPLETE`
again as of the wave-6 record below, on the control being on the shipped screen and driven in a
browser there. And one row was **added**:
`KU-034`, the gateway's cross-entity search, which shipped a wave ago with no row in this file at
all. That is the same class of drift as a stale State cell and it is invisible to a script that
counts the rows it is given, so it is recorded here rather than only in the row.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CL-001 | List clusters with status, counts, throughput, `lastError`, `readOnly` | Kafbat, Provectus, Kouncil | P0 | cluster (+gateway aggregation) | shell, feature-clusters | M1 | M | COMPLETE | Audited 2026-09-04. List, counts and `readOnly` verified live on four clusters. **Version fixed 2026-09-04** (`2d8d0ae`): the level table now runs to Kafka 4.4, a level newer than the table reports the highest known release as a lower bound ("4.4 or newer") rather than nothing, and `KafkaToDomain` no longer discards the resolved numbers to re-parse the broker's words (`level 30`, which cannot parse) — either fault alone emptied the cell. Confirmed on a rebuilt image against the running three-cluster demonstration on 2026-09-04: all three clusters report `4.3`, which is the release the level-30 metadata version names and what `apache/kafka:4.3.1` runs. **Under-replicated** is still `None` by design in M1. The `DISK` column reported the host filesystem (468.8 GiB on the quickstart) until `147461d`; it now reports what Kafka's data occupies. How full the underlying disk is is not shown anywhere. **Verification pass 2026-09-05.** Confirmed again on a rebuilt image: the clusters list and every dashboard card read `4.3` for all three demonstration clusters. `Disk` reads `—` on the first render after start-up and fills in (4.9 MiB / 48.2 MiB / 2.4 MiB) once the first log-dir scrape lands; that is the freshness marker doing its job, not a defect. **Also 2026-09-05:** on a 1565px window this table made the **whole document scroll sideways** -- nine columns wanted 1456px inside a 1197px column, and nothing between the table and `<body>` clipped -- so reaching the TOPICS column carried the sidebar off the left edge, while "Development" simultaneously broke across two lines in a squeezed cell. `DataTable` now returns its table inside an `overflow-x: auto` box (`9b252dd`). The page no longer scrolls horizontally; the columns still do not fit, and the honest cure for that is fewer or narrower columns, which is a design decision and not a CSS one. `UNDER-REPLICATED` reads an em dash on every cluster because `ClusterSnapshots` passes `partitions = None` unconditionally -- recorded above as deliberate for M1, but it means the demonstration's headline scenario (stop a production broker, watch under-replication appear) cannot be shown. |
...
| CL-004 | Cluster-level aggregated metrics (JMX/Prometheus) | Kafbat, Provectus | P1 | metrics | feature-clusters | M8 | M | COMPLETE | **Moved out of `RESEARCHING` 2026-09-07 and to `COMPLETE` the same day, each on the evidence that existed when it was moved.** Wave 5 added the four reads beside `throughput` — `…/metrics/latency`, `…/metrics/request-handlers`, `…/metrics/producers`, `…/metrics/record-size`, each `Section`-wrapped so one dead exporter costs one card (ADR-052) — routed them at the gateway, and built the cards that read them (`frontend/packages/shell/src/overview/`: `LatencyCard.tsx`, `TrafficCards.tsx`, `metrics.ts`). `RESEARCHING` in this file means nothing is built, and that stopped being true when the endpoints landed. What was missing was the only thing that makes a row `COMPLETE` here — **nobody had done it from a browser** — because the quickstart running that morning was built from a tree older than the endpoints and answered `KUI-ROUTE-NOT-FOUND` on all four. It has been done since, against a stack rebuilt from this tree (`kui-frontend:0.1.0-SNAPSHOT` `84a2f1f0cff0`, `kui-allinone:0.1.0-SNAPSHOT` `0b5bba2eed77`): `pnpm exec playwright test traffic.spec.ts` is **10 passed, 1 skipped, 0 failed**, one case per card plus the stat row. The skipped one is the `not_configured` case, which needs a second cluster with no metrics source and the quickstart has one cluster. Two of the five cards drew nothing until wave 6 — the wire the server sent and the wire the browser read had different field names, both sides green and both sides unit-tested against their own shape — so this row's evidence is deliberately what the screen drew and not what the endpoint answered. Three of the design's figures are deliberately *not* drawn as drawn — purgatory is a queue length and not a percentage, top producers is by topic and not by `client.id`, and there is no record-size distribution — each decided in ADR-052 rather than derived and mislabelled. |
...
| CL-012 | KRaft quorum panel: leader, epoch, high watermark, voters and observers with lag | KUI-new | P1 | cluster | feature-clusters | M1 | S | REVIEW | Delivered 2026-09-04. `QuorumDto`/`QuorumMemberDto` in `libs/contracts-core`, carried on `BrokersResponse` beside the broker section (same snapshot pass, so the high watermark and the offsets it is subtracted from are one moment). Every member's lag is computed server-side through `QuorumInfo.lagOf`, never by a client. `QuorumPanel` renders voters and observers separately, marks members that are behind, and states in words whether a strict majority is level with the leader — the sentence that says a metadata write may not be able to commit. A cluster with no quorum renders nothing at all. **Verification pass 2026-09-05.** **Confirmed in a browser.** `/ui/clusters/production/brokers` renders "Metadata quorum": leader 1, epoch 6, committed up to 36259, 3 voters, a voter table with log end offset, behind-by, last fetch and last caught up, and the sentence "3 of 3 voters are level with the leader, so metadata changes can still be committed". Per-broker `Leaders` and `Leader skew` are still `—` on that page, because `leaderCount`/`partitionCount` are `null` on the wire. |

## Brokers (BR)

| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| BR-001 | List brokers with rack, leader counts, skew %, throughput | Kafbat, Provectus, Kouncil | P0 | cluster | feature-clusters | M1 | M | COMPLETE | Audited 2026-09-04. Broker, host, port, rack, disk and replica count render. **Corrected 2026-09-06: `leaderSkewPercent` is filled.** `ClusterTopology.leaderSkewOn` derives it from the partition census through the same `BrokerLoad.skewOf` that produces the replica skew in the adjacent column, and `ClusterMapping` carries it onto the wire (`row.leaderSkewPercent`); **and, since 2026-09-06, a screen draws it**: an expanded broker card states the share in words through `leaderSkewSentence`, and says it was not measured when the census could not answer, which `frontend/e2e/brokers.spec.ts`'s *says a broker's share of the leaderships, or says it was not measured* drives in a browser. `leaderCount` and `partitionCount` come from that same census (`ClusterTopology.leadersOn`/`partitionsOn`) and render `—` only when the topic sweep could not describe every topic. `segmentCount` alone is still hard-coded `None` (`ClusterMapping`) and always renders `—`. Throughput needs M8. The `inSyncReplicaCount` → `replicaCount` rename (147461d) is source-only; no running image has it. The broker list's `DISK` column had the same defect and the same fix (`147461d`). |
| BR-002 | Broker configs with source, sensitivity, read-only flag, synonyms | Kafbat, Provectus, Kouncil | P0 | cluster | feature-clusters | M1 | S | COMPLETE | Audited 2026-09-04. **Recounted 2026-09-07: 341 config entries**, not the 340 this row carried from a wave plan to a wave plan without anybody asking the broker. Measured against the running quickstart with `curl -s .../clusters/quickstart/brokers/1/configs | jq '.configs.data | length'` → `341`, in a 61,671-byte response; the entries carry source, sensitivity, read-only and synonyms. Redaction is proved by `SecretLeakSuite`, never observed on a live broker (none reported a sensitive setting). Synonyms are carried and rendered nowhere (TD-019). |
...
| TP-006 | Update topic configs (diff + incremental alter) | Kafbat, Provectus | P0 | topic | feature-topics | M5 | M | COMPLETE | Audited 2026-09-04. `PATCH …/config` with `set`/`remove`; removing an override restores the broker default rather than freezing its current value. No diff preview: the dialog edits one key at a time. |
| TP-007 | Delete topic (gated by broker `delete.topic.enable`) | Kafbat, Provectus, Kouncil | P0 | topic | feature-topics | M5 | S | COMPLETE | Audited 2026-09-04. Plan-token gated (ADR-045): the plan reports records lost and `auto.create.topics.enable`; a missing or altered token is refused. The `delete.topic.enable=false` refusal is mapped but has never been driven against a broker configured that way. |
| TP-008 | Recreate topic (delete + create with same config) | Kafbat, Provectus | P1 | topic | feature-topics | M5 | M | RESEARCHING | |
| TP-009 | Clone topic to a new name | Kafbat, Provectus | P1 | topic | feature-topics | M5 | S | RESEARCHING | Body `{newName}` instead of query param. |
| TP-010 | Increase partition count | Kafbat, Provectus, Kouncil | P0 | topic | feature-topics | M5 | S | COMPLETE | Audited 2026-09-04. Plan-gated; decrease refused with a typed error; the count is re-resolved at apply time so a replayed token is refused. |
| TP-011 | Change replication factor (reassignment plan) | Kafbat, Provectus | P1 | topic | feature-topics | M5 | L | RESEARCHING | |
| TP-012 | Topic analysis (full scan: counts, sizes, percentiles, HLL uniques, hourly histogram) | Kafbat, Provectus | P1 | topic | feature-topics (Statistics tab) | M5 | L | RESEARCHING | Allowed in read-only mode. Progress over SSE is a later extension. |
| TP-013 | Active producers (idempotent / transactional state per partition) | Kafbat, Provectus | P1 | topic | feature-topics | M5 | S | RESEARCHING | |
| TP-014 | Topic → related connectors tab | Kafbat | P2 | connect (queried by gateway) | feature-topics (FeaturePanel) | M7 | S | RESEARCHING | Section of the topic overview aggregation (KU-011). |
| TP-015 | Topic → related consumer groups tab | Kafbat, Provectus | P0 | consumer | feature-topics (FeaturePanel) | M4 | S | COMPLETE | Audited 2026-09-04. `ConsumerGroupsSource` is registered in `GatewayWiring`; the tab is declared statically in `ConsumersRoutes.guestTabs`, so it no longer depends on browsing history. Live: the topic overview returns a populated `consumerGroups` section. Driven in a browser 2026-09-04 (closing pass): the tab shows `order-fulfilment` with its lag on this topic. Its URL was added in the same pass — the tab existed and `…/topics/{t}/consumers` was a 404. |
| TP-016 | Topic → ACLs tab | Kafbat | P2 | security | feature-topics (FeaturePanel) | M7 | S | RESEARCHING | Needs Topic VIEW and ACL VIEW. |
| TP-017 | Topics CSV export | Kafbat | P2 | topic | feature-topics | M5 | S | RESEARCHING | Content negotiation. |
| TP-018 | Batch actions on the topic list (multi-select delete / purge / copy) | Kafbat, Provectus | P1 | — (client composition) | feature-topics | M5 | M | RESEARCHING | Partial failure reported per topic. |
...
| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MS-001 | Stream messages over SSE with polling modes (from/to offset, from/to timestamp, latest, earliest, tailing) | Kafbat (v2), Provectus (v1) | P0 | message | feature-messages | M3 | XL | COMPLETE | Audited 2026-09-04. Forward and backward, `beginning`/`latest`/`offset::N`/`timestamp::ms` and per-partition `seekTo[]` all verified live. **Tailing is not among them — see MS-005.** |
| MS-002 | Cursor-based next page over the stream | Kafbat | P0 | message | feature-messages | M3 | M | COMPLETE | Audited 2026-09-04. The `done` event's signed cursor is sent back by `BrowseSession.loadMore` behind the `messages-load-more` button; a second page was read live. No e2e test covers it. |
| MS-003 | Table-style browsing with per-partition, newest-first offset paging | Kouncil | P0 | message | feature-messages | M3 | L | RESEARCHING | Second, non-streaming read path (D-1). Kouncil-defining UX. Not started; the M3 screen is the streaming list only. |
| MS-004 | JSON column flattening grid (headers / key / value columns, depth and row caps) | Kouncil | P0 | — (client-side) | feature-messages | M3 | L | COMPLETE | Delivered 2026-09-04. `FlatTable` renders the flattener's output: `H.*` / `K*` / `V*` columns in first-seen order (so a record arriving on a tail adds columns on the right and never reorders the ones being read), a `<details>` column picker holding what is *hidden* rather than what is chosen, a record with no value for a column leaving a gap rather than shifting its neighbours, and the row cap stated under the table. `RecordSource` is the `MessageDto` → `FlatSource` conversion, including the not-JSON degraded case. A `view=table` parameter puts the choice in the URL, so a table view is a link. 7 DOM tests. **Verification pass 2026-09-05.** **Confirmed in a browser** against `orders.v1`: pressing Table put `view=table` in the URL and produced 44 columns — `Partition`, `Offset`, `Timestamp`, `H.event-type`, `H.trace-id`, `K`, `V.orderId`, `V.items[0].sku` and the rest — in first-seen order, with gaps where a record has no value for a column. |
| MS-005 | Live / tailing mode with client throttle and play/pause | Kafbat, Provectus, Kouncil | P0 | message | feature-messages | M3 | M | COMPLETE | Delivered 2026-09-04. `BrowseRequest.live` now reaches the poll loop: a tail always reads forwards (the default direction of `Latest` is backwards, and backwards from the end of the log is the empty range that made the control deliver nothing), its window has no upper bound, and none of the three things that end a bounded read — the limit, consecutive empty polls, the budget deadline — ends a tail. It stops on cancellation only, which closes the consumer. Play/pause is client-side in `BrowseSession`: paused holds records back with the stream still open and the count still moving, resuming shows them in order, Stop releases what was held. 24 tests across the domain, application, infrastructure and `feature-messages` suites. Throttle (`PollBudget.throttleBytesPerSecond`) remains, with MS-014. **Verification pass 2026-09-05.** **Confirmed streaming in a browser.** With `Follow live` ticked the button became Stop, a Pause appeared, and the status read `Reading · 0 records` indefinitely; two records produced into `orders.v1` from a shell appeared within about two seconds. Pause held the next record back while the count kept moving (`Reading · 3 records · 1 record waiting`) and Resume showed it. |
| MS-006 | Simple string-contains filter over key, value, headers | Kafbat, Provectus, Kouncil | P0 | message | feature-messages | M3 | S | COMPLETE | The `Contains` box on the message screen; `q=OrderShipped` verified against the quickstart broker. |
| MS-007 | Smart filters: CEL scripts, registration with TTL, dry-run test, saved filters | Kafbat (CEL), Provectus (Groovy) | P1 | message | feature-messages | M3 | L | COMPLETE | Delivered 2026-09-04, except the saved-filter *list*. `libs.filter` is now a dependency of `services.message.infrastructure`, where `CelFilterSource` is the only file that names CEL: it satisfies the domain's `FilterSource` port, which `BrowseUseCase` compiles once per request before any consumer is opened and then evaluates per record. The three-way verdict survives end to end — a record the expression threw on is excluded, counted, and reported as `consumed.filterErrors`. `POST …/messages/filters` registers (and thereby validates, returning `KUI-FILTER-COMPILE` with line and column) and `…/filters/test` tries an expression against a record the caller sent; both are in the gateway's contract map and both are bound through `SecuredRoutes.withBody`. The browser has `FilterApi` and a `FilterEditor` whose Apply registers and then writes `filterId`+`filterSource` into the URL, so a filtered browse is a link. 19 tests. Remaining: a stored list of named filters. **Verification pass 2026-09-05.** **Confirmed filtering in a browser** against a real broker: `record.value.currency == 'EUR'` registered (`filterId=b8e2269e70d7fa00` appeared in the URL) and the next Read returned 4 rows where the unfiltered read returned 16, every one of them EUR. |
| MS-008 | Purge messages (delete records per partition) | Kafbat, Provectus | P1 | message | feature-messages, feature-topics | M3 | S | COMPLETE | Audited 2026-09-04. Plan-gated purge, owned by the message service, with the control on the *topic* page (`TopicAdminPanel.purgeSection`). Plan and apply both driven against a broker. A compacted topic has never been purged on screen. |
| MS-009 | Serde suggestions per topic for serialize / deserialize | Kafbat, Provectus | P0 | message | feature-messages | M3 | S | IMPLEMENTING | Audited 2026-09-04. `ClusterSerdes.suggest` and `SerdeSuggestionDto` exist; no endpoint exposes them. |
| MS-010 | Message detail view (key / content / headers, copy, pre-masking raw, resend entry point) | Kafbat, Provectus, Kouncil | P0 | — | feature-messages | M3 | M | REVIEW | Audited 2026-09-04. Key, pretty value, headers, decode errors and row actions render. No copy-to-clipboard, no pre-masking raw view, no per-record serde picker. |
| MS-011 | Export rendered messages to CSV / JSON | Kafbat | P2 | — | feature-messages | M3 | S | REVIEW | Delivered 2026-09-04, client-side. `kui.ui.kernel.file.Csv` is the shared RFC 4180 writer (OT-002's home: quoting, CRLF, and the `=`/`+`/`-`/`@` formula-injection guard) and `Download` hands the file over through a Blob and a revoked object URL. `kui.ui.kernel.file.Csv` and the whole citation in this entry name the Scala.js implementation ADR-048 deleted; the feature was ported to `frontend/packages/` and this evidence has not been re-taken. `RecordCsv` exports what is on screen in the shape of the view the user is in — the record in the list view, the chosen grid columns in the table view. Client-side because the export is of *rendered* records: re-reading the topic would produce a different file. The audit's note that a sandbox blocks page-initiated downloads describes the artifact viewer, not KUI, which is served to an ordinary browser. A server-rendered export larger than one page remains KU-030. 15 tests. **Verification pass 2026-09-05.** **The export handed the browser no file, and was fixed (`8c45bf3`).** The anchor was built with `dataAttr("download")`, which sets `data-download`; the attribute that makes a browser save, `download`, was never set at all. Observed in a browser: the click produced exactly the right CSV bytes — header row, CRLF, correct RFC 4180 quoting of the embedded JSON — and the anchor's `download` property read as the empty string, so there was no named file to find. `DownloadSuite` now asserts the attribute, because nothing in the type system tells `download` from `data-download`. Back to `REVIEW`: the fix has not been driven in a browser. |
...
| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MP-001 | Produce a message (key, value, headers, partition, serde selection, keep-contents) | Kafbat, Provectus, Kouncil | P0 | message | feature-messages | M3 | M | COMPLETE | Publish drawer, plus Republish and Copy-to-another-topic on an open record. Driven in a browser against the quickstart broker on 2026-09-04: published a keyed JSON record with a header, got back `partition 3, offset 3`, and read it back from the browse screen. A read-only cluster is refused before a producer is asked for. Serde selection per MP-004 is not built. **Verification pass 2026-09-05.** **Publishing from the browser was impossible, and was fixed (`4c1e1a8`).** The value box accepted exactly one character: typing `{"orderId":"x"}` left `{` in it and moved the caret to the drawer's close button, so pressing Publish wrote an empty record -- which is what it did the first time it was tried against the demonstration. The fault was not in the form. `Drawer` (and `Dialog`) rendered their panel through `child.maybe <-- open.signal.map(...)`, and an Airstream `Var` emits on every `set` including a `set` to the value it already holds; `ProduceDrawer` writes `open.set(draft.isDefined)` on every keystroke, so each character rebuilt the whole panel and destroyed the focused textarea. Fixed with `.distinct` in the two components, with three tests watched failing first. Re-driven end to end afterwards: `{"orderId":"ORD-VERIFY-2"}` published to `orders.v1` on the development cluster, `Published 1 record. partition 1, offset 6`, and read back off the broker with `kafka-console-consumer`. **Two things left open:** a stale CSRF token (the server restarted under an open page) fails the publish with the raw string `X-Csrf-Token does not match the session's token` and no recovery -- a header name shown to an operator, with no offer to reload; and both that error and the success confirmation render below the fold of the drawer, so the outcome of a write is off screen unless the drawer is scrolled. |
| MP-002 | Bulk send with `{{count}}`, `{{timestamp}}`, `{{uuid}}` placeholders | Kouncil | P1 | message | feature-messages | M3 | S | IMPLEMENTING | Audited 2026-09-04. Server-side `count` works (two records acked from one request, live). The `{{count}}`/`{{uuid}}`/`{{timestamp}}` placeholders are specified as a browser feature and are not implemented in the browser. |
| MP-003 | Resend (copy) an offset range between topics with header filtering | Kouncil | P1 | message | feature-messages | M3 | M | COMPLETE | Audited 2026-09-04. **Corrected upward from 'not built'.** `POST …/messages/resend` copied a three-record range between topics live. Kouncil's header *filtering* is absent: `ResendRequestDto` carries only `toTopic` and `ranges`. |
| MP-004 | Produce with per-serde parameters and schema validation before send | Kafbat | P1 | message | feature-messages | M3 | M | RESEARCHING | Uses SD-004. |

## Consumer groups (CG)

| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CG-001 | List consumer groups: paged, sorted, searched, state filter | Kafbat, Provectus, Kouncil | P0 | consumer | feature-consumers | M4 | M | COMPLETE | Paged, searched and state-filtered from a browser. Verified against the quickstart's three seeded groups. |
| CG-002 | Group details: members, assignments, offsets, lag, pace | Kafbat, Provectus, Kouncil | P0 | consumer | feature-consumers | M4 | M | COMPLETE | Audited 2026-09-04. Members, assignments, offsets and lag render. **The 'pace is null, nothing measures it' note was wrong**: `LagMath.pace` is computed and served on the list and lag-delta payloads (live: `-9.296…`). **Pace column delivered 2026-09-04** (`05287d2`): a non-sortable column on the group list, one decimal place, `< 0.1` rather than `0.0` for a slow group, a muted `0` with "committing nothing" for a stalled one, and a coloured negative for offsets moving backwards. The detail DTO still omits it. **Verification pass 2026-09-05.** A `PACE` column is on the group list and reads `0` for every group in the demonstration, including a `Stable` group with three members that committed roughly two thousand offsets while the page was open. The wire carries `"pace": 0.0` — a number, not `null` — so the two-sample comparison ran and produced zero rather than declining to answer. Whether that is honest (the demonstration consumers commit in bursts, and both samples can land inside one quiet stretch) or a defect in the sampling was **not settled**: the environment was rebuilt by another lane before a second measurement could be taken. Nothing on screen has yet been seen to show a non-zero pace. |
| CG-003 | Delete group | Kafbat, Provectus, Kouncil | P0 | consumer | feature-consumers | M4 | S | COMPLETE | Delivered 2026-09-04 (`05287d2`). `GroupDangerZone` on the group detail page: a confirmation dialogue whose default focus is Cancel, no plan token (a delete has no arithmetic to preview), and no client-side copy of Kafka's not-empty rule — the server re-checks it immediately before the write and a consumer can join between the click and the request, so the refusal is rendered as the sentence it is. **Verification pass 2026-09-05. Confirmed end to end against a real broker:** pressing Delete the group on `dlq-triage` in the demonstration's production cluster removed it, and `kafka-consumer-groups.sh --list` on the broker afterwards showed six groups where it had shown seven. |
| CG-004 | Reset offsets wizard (earliest / latest / timestamp / per-partition offset) | Kafbat, Provectus, Kouncil | P0 | consumer | feature-consumers | M4 | M | COMPLETE | Audited 2026-09-04. All six modes planned live and apply is token-only. Two parity gaps against the references: no partition-subset selection (every partition of the topic is always sent) and `OFFSET` mode replicates one offset to every partition. |
| CG-005 | Delete committed offsets for one topic | Kafbat | P1 | consumer | feature-consumers | M4 | S | COMPLETE | Delivered 2026-09-04 (`05287d2`). Offered per topic, on the topics the group actually holds offsets on, with a receipt that names how many partitions were forgotten — so 'the group held none there' and 'they are gone now' stay different sentences. **Corrected downward 2026-09-06 and back up 2026-09-07, and both halves of the downward note are now false of the tree.** The 2026-09-06 pass recorded that no file under `frontend/` carried the string *Forget this group's offsets on one topic* and that a repository-wide search for `deleteOffsets` found one reference, its own declaration; that was true of the tree it read. It is not true of this one: the heading is at `frontend/packages/feature-consumers/src/GroupDetail.tsx:171` and `GroupRoute.tsx:94` calls the function declared at `write.ts:177`, so the search finds three references and not one. The row's exit criterion was *the group detail page offers it per topic and the receipt is asserted at the route*, and both halves are met. Measured 2026-09-07 against the running quickstart (`kui-frontend:0.1.0-SNAPSHOT` `84a2f1f0cff0`, `kui-allinone:0.1.0-SNAPSHOT` `0b5bba2eed77`): `pnpm exec playwright test consumers.spec.ts` is 6 passed / 0 failed, and the sixth is *offers forgetting the offsets on each topic the group holds them on*, which reads the coordinator's own topic list over the API and asserts one row per topic with that topic's partition count beside it. The destructive half is driven at the route rather than in the browser — forgetting a seeded group's offsets removes the group, and KUI cannot create one to do it to — in `src/groupRoute.test.tsx`, whose case *forgets one topic's offsets and reports how many partitions the server removed* asserts the receipt quotes the server's count and not the browser's |
| CG-006 | Incremental lag polling and lag trend sparkline | Kafbat | P1 | consumer | feature-consumers | M4 | M | REVIEW | Audited 2026-09-04. **The 'nothing calls it, the list does not poll' note was wrong.** `GroupListPage` mounts `LagPoller.binder`; the endpoint answers with a token and `nextPollMs` live. What remains is GRP-033 items 3–4: `LagTrend`, `LagSparkline`, the ▲/▼ arrows and the missing-sample rules — no `lag/` directory exists. |
| CG-007 | Consumer groups CSV export | Kafbat | P2 | consumer | feature-consumers | M5 | S | RESEARCHING | |
...
| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SR-001 | List subjects: paged, sorted, searched | Kafbat, Provectus, Kouncil | P0 | schema | feature-schemas | M7 | M | COMPLETE | Updated 2026-09-04, **corrected 2026-09-06: the microfrontend exists.** `GET /api/v1/clusters/{id}/schemas/subjects` is searched, sorted and paged in `SubjectCatalog` because the registry offers none of the three. `feature-schemas` is registered in `shell/src/features/registry.ts`, routed at `/clusters/:id/schemas`, and driven in a browser by `frontend/e2e/features.spec.ts`, which asserts a subject name and the registry's global compatibility level in words. `SubjectList` renders the search field and the pager and states a `NONE` global level as a sentence rather than only as a colour. **Completed 2026-09-06: both gaps this row named are closed.** Sorting reaches the request — `SchemasRoute` holds the `direction` signal and puts it in the query it sends, asserted at the route by `schemas.test.tsx`'s *puts the sort control's direction into the request it sends the registry*, which goes red when the value is hard-coded (mutation run 2026-09-06). The two-pane master–detail is built as `SchemaWorkspace`, and `frontend/e2e/features.spec.ts`'s *selecting a subject changes the address and keeps the list beside it* drives it in a browser against a running stack. |
| SR-002 | Create schema / new version (Avro, JSON Schema, Protobuf) | Kafbat, Provectus, Kouncil | P0 | schema | feature-schemas | M7 | M | COMPLETE | **Delivered 2026-09-06 (ADR-051).** This was M6's one bullet with no backend capability behind it, and the control shipped `aria-disabled` with a true sentence for two waves. `POST …/clusters/{id}/schemas/subjects/{subject}/versions` takes the document and its `schemaType` and answers the registry's id and, where the registry reports one, the version — absent rather than `0` when it stores the schema and will not say which version it became. Authorized by `Action.SchemaCreate`, which had existed since wave 1 with no caller; refused on a read-only cluster with `KUI-READ-ONLY` before the registry is asked; a rejection is a 400 `KUI-VALIDATION` carrying **the registry's own sentence** in `details[0]`, which `RegisterSchemaDialog` renders rather than paraphrases. All three formats the registry accepts are offered (`AVRO`, `JSON`, `PROTOBUF`). Driven in a browser against a running stack on 2026-09-06 — `frontend/e2e/features.spec.ts`'s *registers a schema, and the registry's answer is what confirms it* — and the subjects it wrote are in the quickstart registry. **Parity gaps:** only Avro has been registered against a real registry, and a Protobuf or Avro schema carrying `references` whose version does not parse has them silently dropped rather than refused |
...
| SR-009 | Latest key/value schema for a topic (feeds the produce editor) | Kouncil | P1 | schema | feature-messages | M7 | S | RESEARCHING | `GET /topics/{topic}/schemas`; section of the topic overview aggregation. |
...
| KC-010 | Connect / connector CSV export and connect client cache | Kafbat | P2 | connect | feature-connect | M7 | S | RESEARCHING | The n-gram search part follows DR-10 (M9). |
...
| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AC-001 | List ACL bindings with resource / name / pattern filters | Kafbat, Provectus | P1 | security | feature-security | M7 | M | RESEARCHING | `MATCH` exposed as a filter-only value. |
| AC-002 | Create / delete ACL binding | Kafbat, Provectus | P1 | security | feature-security | M7 | S | RESEARCHING | Delete via `POST /acls/delete` (no DELETE with body). |
| AC-003 | ACL CSV export and declarative CSV sync | Kafbat, Provectus | P1 | security | feature-security | M7 | M | RESEARCHING | Sync is destructive: confirmation with typed name. |
| AC-004 | Convenience creators: consumer, producer, stream app | Kafbat, Provectus | P1 | security | feature-security | M7 | M | RESEARCHING | |

## Client quotas (QU)

| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| QU-001 | List client quotas (user / client id / ip) | Kafbat | P1 | security | feature-security | M7 | S | RESEARCHING | Screen 26 of the IA proposal. |
| QU-002 | Upsert / delete quotas | Kafbat | P1 | security | feature-security | M7 | S | RESEARCHING | |

## Metrics, graphs, Prometheus (MT)

| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MT-001 | JMX metrics scraping (SSL, auth) per broker | Kafbat, Provectus, Kouncil | P1 | metrics | — | M8 | L | RESEARCHING | **Declared and deliberately not implemented (ADR-050, 2026-09-06); that is not the same as untouched.** `MetricsSourceKind.Jmx` is a case of the configuration vocabulary, and a cluster configured with it answers a stated refusal naming the build rather than throwing or silently reading nothing. It cannot be implemented behind the settings as they stand: `MetricsSourceSettings.url` is a `SafeUrl`, which `ARCHITECTURE.md` §14 restricts to `http` and `https`, so `service:jmx:rmi:///jndi/rmi://kafka:9999/jmxrmi` cannot be written into a KUI configuration file at all. Implementing this row means a new settings shape, not a new adapter behind the same one — and it is not closed by deleting the case |
| MT-002 | Prometheus endpoint scraping | Kafbat, Provectus | P1 | metrics | — | M8 | M | COMPLETE | **Delivered 2026-09-06 (ADR-050).** `PrometheusExposition` parses a JMX-exporter text exposition for the broker-wide `BytesInPerSec` and `BytesOutPerSec` families and ignores families it does not know rather than failing on them; `PrometheusBrokerScrape` fetches it inside `callTimeout`; `BrokerScrapeLoop` runs one fibre per configured cluster at `kui.metrics.scrapeInterval` into a `SeriesWindow` buffer whose never-sampled buckets are **absent**, never zero, and which a failed scrape does not evict. The quickstart runs a `jmx-exporter` sidecar and names it in `kui.metrics.sources.quickstart`. Measured on that stack 2026-09-06: `GET …/clusters/quickstart/metrics/throughput?range=24h` answers `"status":"ok"` with **288 buckets of which 13 carry a rate** — 288 is `ThroughputRange.Last24Hours.bucketCount` and not the number of samples, which is the rule the axis rests on. The dashboard's Traffic tab draws it, and a null bucket is a gap in the line rather than a zero. **Corrected 2026-09-07:** the sentence that stood here — that latency, request handlers, top producers and record size are four endpoints that do not exist — stopped being true when wave 5 added them (ADR-052). One scrape now fills one `BrokerSample` from which all five reads are views, so two cards on one screen cannot disagree about when *now* was. What this row claims is unchanged and still measured: a Prometheus exposition is scraped, parsed and served. Whether the four new cards draw it in a browser is `CL-004`'s question, not this one's |
...
| MT-004 | Graph descriptions and Prometheus query proxy with templated PromQL | Kafbat | P2 | metrics | feature-metrics | M8 | L | RESEARCHING | Typed `GraphData` instead of raw PromQL passthrough. uPlot facade (ADR-025). |
...
| CW-002 | Read current config (redacted) and apply a new one with hot reload | Kafbat, Provectus | P1 | cluster + identity, aggregated by gateway | feature-admin | M8 | L | RESEARCHING | No process restart: config distribution to services. `/api/v1/config` is a gateway aggregation over the two owners (ADR-004, ADR-036). |
| CW-003 | Validate a config with per-component connectivity probes | Kafbat, Provectus, Kouncil | P1 | cluster | feature-admin | M8 | M | RESEARCHING | Probe URLs pass the SSRF policy (KU-024). |
| CW-004 | Upload related files (truststore, keystore, proto) | Kafbat, Provectus | P2 | cluster | feature-admin | M8 | S | RESEARCHING | Bytes go to the `__kui_files` topic, encrypted, size-capped by `kui.store.maxFileBytes` (ADR-042). |
| CW-005 | Cluster config wizard UI (bootstrap, auth, SR, Connect, ksql, metrics, serdes, masking) | Kafbat, Provectus, Kouncil | P1 | — | feature-admin | M8 | L | RESEARCHING | Screens 28–29 of the IA proposal. |
...
| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AU-001 | Auth types: disabled, login form, OAuth2/OIDC (GitHub, GitLab, Google, Cognito, Azure Entra, generic), LDAP / AD | Kafbat, Provectus, Kouncil | P0 | identity (+gateway session) | shell | M6 | L | REVIEW | Backend delivered 2026-09-04. `disabled` (still the default), `form` and `oidc` are real: `kui.auth.type` is read, the identity service verifies passwords and completes an OpenID Connect flow, and the gateway serves `/api/v1/auth/{settings,login,password,oidc/start,oidc/callback}` with a session issued on success. LDAP and Active Directory are **not** implemented and are refused by name at start-up (ADR-015 Amendment 1); per-vendor group extractors are RB-002. **Verification pass 2026-09-05: the sign-in screen now exists and works.** *(That pass ran against the Scala.js shell, which ADR-048 deleted on the same day; the screen is now `frontend/packages/shell/src/pages/SignIn.tsx` and has not been re-driven in a browser.)* What that pass drove was a Laminar `LoginPage` under the `frontend/ui-shell/` tree, and **neither the tree nor the file is in this repository** — `git ls-files frontend/ui-shell` answers nothing — so this row records what was verified and no longer cites a path a reader could try to open. It rendered a username-and-password form for `form`, a single provider button for `oidc`, and the required-password-change step as its own state. Signing in as `viewer` against `quickstart.sh --with-auth` was done in a browser end to end: the screen appears, the credentials are accepted, the header carries the name, the role's permissions gate the controls and Sign out returns to the screen. Three defects had to be fixed to get there, all of them invisible to the green suites that shipped the backend — a `Set-Cookie` output a browser can never decode, a screen rebuilt by the connectivity ticker that emptied its own fields, and a form Enter would not submit. **Still open:** LDAP and Active Directory are not implemented and are refused by name at start-up (ADR-015 Amendment 1); the OIDC flow has a screen but has not been driven against a real provider; per-vendor group extractors are RB-002. |
| AU-002 | In-memory users with default accounts and forced password change | Kouncil | P2 | identity | shell | M6 | M | REVIEW | Delivered 2026-09-04, backend only. `kui.auth.users[]` holds a name, a PBKDF2 hash (`kui.identity.app.HashPassword` writes one), groups and `mustChangePassword`; the forced change is *enforced* — such an account gets a single-use challenge and no session until it has set a new password. The change is saved in the metadata store, and a deployment with none is told to configure `kui.store.dir` rather than losing the change silently. **2026-09-05:** the screen exists and form sign-in with a file-configured account is verified in a browser (`quickstart.sh --with-auth`, accounts `admin` and `viewer`). The forced-change step is rendered and unit-tested, and has **not** been driven live: the demonstration configures no metadata store, so the server would refuse the change by design. |
...
| AU-005 | User menu: logout, theme auto/light/dark, timezone | Kafbat | P1 | — | shell | M1 | S | REVIEW | Audited 2026-09-04, **corrected downward from COMPLETE**, partly recovered on 2026-09-05, and **re-read against the tree on 2026-09-12**. The account menu exists as `frontend/packages/shell/src/chrome/AccountMenu.tsx`: it names the signed-in user, says which mechanism signed them in, and offers **Sign out**, which posts to `/api/v1/auth/logout` and reloads the page — a reload rather than a list of stores to clear, because the list is the thing somebody eventually fails to keep up to date and its failure mode is the next person reading the previous person's data. A *failed* sign-out keeps the panel open with the reason and does **not** reload, because reloading on a cookie that is still there shows a signed-in shell and reads exactly like success. It is not rendered at all for an anonymous principal, so the no-login deployment is unchanged. The three-state theme control ships, in `chrome/AppearancePopover.tsx` rather than in this panel. **Two things this row said until 2026-09-12 and the tree does not support.** It named `layout/UserMenu.scala`, in the present tense, for a Scala.js file ADR-048 deleted: `git ls-files | grep -i usermenu` answers nothing. And it said the menu *lists the roles they hold* — the Scala.js panel did, the TypeScript port does not, and `grep -rn roles frontend/packages/shell/src` finds the word only in a comment in `SignIn.tsx`. **Still missing: the timezone and refresh-rate controls** (KU-012), and the role list the sentence above used to promise. |

## Authorization: RBAC and read-only (RB)

| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RB-001 | Role model: subjects × clusters × resource pattern × actions, with action dependencies | Kafbat, Provectus | P0 | identity (evaluation) + gateway (enforcement) | feature-admin (view) | M6 | XL | REVIEW | `kui.rbac.roles[]` and `kui.rbac.defaultRole` are read into `RbacPolicy` at start-up (2026-09-04), with an unknown action, an unnamed pattern on a named resource, an uncompilable regular expression and a duplicate role name all refused as configuration errors naming the key. Roles are resolved once at sign-in and travel in the session and the signed principal. Enforcement at the edge is `PolicyRbacPreCheck`, and the topic service re-checks at its own port. **Verified end to end on 2026-09-05** against `quickstart.sh --with-auth`: signed in as `viewer` (`TOPIC: [VIEW, MESSAGES_READ]`, `CONSUMER: [VIEW]`), a produce and a topic create are both refused with `KUI-FORBIDDEN` and one sentence that names no role, pattern or resource, while the topic list and a browse are served. The screens that manage roles are RB-004. |
...
| RB-003 | Current user info and flattened permissions for UI gating | Kafbat, Provectus | P0 | gateway | kernel | M6 | S | COMPLETE | `GET /auth/me` now answers with the real principal, its roles, and the expanded permission list (`Rbac.grants`), and `GET /auth/settings` says which sign-in this deployment uses. A deployment with no roles still answers a wildcard grant per resource rather than an empty list, so nothing is hidden in the quickstart. **The browser half is delivered on 2026-09-05, and it had to be, because the gap was shipping broken controls.** Signed in as `viewer`, the interface offered eight controls the server refuses — "New topic", "Publish", "Republish", "Resend", "Delete group", "Forget offsets", "Reset offsets" — each verified refused with `curl` before being gated. `PermissionStore.allowsAny` was added for the question a *create* asks (a name that does not exist yet cannot match a grant's pattern), mirroring the gateway's own body-named rule exactly so the two cannot disagree. Nine new tests. Verified in a browser: the create button is disabled and its tooltip reads "You do not have permission to do this." |
| RB-004 | UI-managed user groups and function-permission matrix, persisted | Kouncil | P1 | identity | feature-admin | M6 | L | RESEARCHING | Persisted role store behind the same evaluation API; file wins, UI adds (D-5). Stored under the `rbac/roles` key of `__kui_config` through OT-004 (ADR-042), which is already available from M1. |
...
| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SD-001 | Core built-in serdes: String, Schema Registry (Avro / JSON Schema / Protobuf), Int32/64, UInt32/64, Base64, Hex, UUID; auto-detection by magic byte | Kafbat, Provectus, Kouncil | P0 | message | feature-messages (picker) | M3 | XL | COMPLETE | Updated 2026-09-04. Wired: the message service's composition root now builds a `SchemaRegistrySerdeFactory` for every cluster that configures `schemaRegistry`, so registry payloads decode in the product and not only in a suite. **Protobuf now decodes too**: `ProtoSchema` parses the `.proto` text the registry returns and `ProtobufPayload` reads the record against it, with Confluent's message-index prefix handled — no dependency and no Confluent Community Licensed code (ADR-014 Amendment 2). Writing Protobuf is still refused by name; Avro and JSON Schema encode as well as decode. Verified live against Apicurio Registry with records written by the reference `confluent-kafka` producer. **Verification pass 2026-09-05.** The quickstart's Avro seed had never once succeeded — it ran `curl` in `apache/kafka:4.3.1`, an Alpine image with no `curl` at all, and reported the failure as "the registry refused the schema". Fixed; five Avro records are now produced and read back as decoded JSON in the browser, `"serde":"SchemaRegistry"`. **Honest degradation was also wrong and is fixed**: with the registry stopped, a record kept its bytes and said "the payload is not valid UTF-8", which is true of the fallback's attempt and points the reader at their data. It now reads "the SchemaRegistry serde is configured for this topic and could not be used: …", and the capability row for the cluster reports `degraded` with the same upstream named. **The demonstration environment now has a registry too (2026-09-05).** It had none at all: its `_schemas` topic held records that looked like a registry's but no registry had written them, so no screen in the demo exercised the Avro path. `docker-compose.demo.yml` now runs a real Confluent Schema Registry on the Development cluster and seeds `orders.avro.v1` with eight Avro records; browsing it returns `"serde":"SchemaRegistry"`, `"type":"AVRO"` and decoded JSON, and `/clusters/development/schemas/subjects` lists the subject beside the three the seed file carries. Production and Secured report `not_configured`, which is the other half of the behaviour. |
...
| SD-004 | Schema → JSON Schema conversion for produce-form validation | Kafbat, Provectus | P1 | message | feature-messages | M3 | M | RESEARCHING | |
...
| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ET-001 | Track a business event across topics by header or value match within a time window (sync, capped) | Kouncil | P0 | message | feature-messages (Track page) | M3 | L | COMPLETE | Delivered 2026-09-04. `TrackUseCase` reads each named topic forwards from the start of the window through the same `RecordSource` and serdes a browse uses — so what a track matches on is exactly what the browse screen would have shown — and `PreparedMatch` compiles the pattern once for the whole scan. Topics are read one at a time: six consumers at once is six times the load on a cluster somebody is already investigating, and sequential is what makes the `limit` ending mean "the first N in this topic order". `POST /api/v1/clusters/{id}/messages/track` answers with the hits, `scanned`, `matched` and `truncated` — `scanned` because "nothing matched" and "nothing was read" mean opposite things, and `truncated` because a caller cannot infer it by counting. 8 use-case tests. **Verification pass 2026-09-05.** **Confirmed in a browser.** `/ui/clusters/development/messages/track`, topic `orders.v1`, value contains `ORD-10246`, window 2026-09-01 to 2026-09-06: `3 hits in 19 records read`, and the three hits are that order's placed, paid and shipped events in offset order. |
| ET-002 | Track asynchronously with results streamed as found | Kouncil | P0 | message | feature-messages | M3 | M | RESEARCHING | Not built, and cheap now: `TrackUseCase` already *is* a stream of `Hit` / `Scanned` / `Finished` events, so what remains is an SSE endpoint over it and a second stream relay in the gateway beside `MessageStreamRoutes`. ET-001's synchronous endpoint is the right answer for a narrow window and the wrong one for a wide one, which is what this is for. |
| ET-003 | Track filter UI (field / operator / value, topic multi-select, time range) and results grid | Kouncil | P0 | — | feature-messages | M3 | M | COMPLETE | Delivered 2026-09-04. A second page in the messages feature at `/ui/clusters/{id}/messages/track`, reached from the browse screen, with its own `history.state` codec. `TrackForm` is the pure half — topics split on commas or whitespace, the window parsed the same way the browse's timestamp box parses it, and every refusal a sentence about the control that caused it, including the header-search-with-no-header-name that the reference product silently turns into a value search. Results are one table sorted by timestamp across topics, with the topic on every row. 19 tests. **Verification pass 2026-09-05.** Confirmed in a browser with the run above: topics, look-in, compare, value and the two window fields all bind, and the results grid renders topic, partition, offset, timestamp, key and value. |
...
| KU-004 | Feature registration contract and lazy feature loader (Vite code splitting, ADR-012 as amended by ADR-048) | KUI-new | P0 | — | kernel, shell | M0 | M | COMPLETE | Audited 2026-09-04 against the Scala.js build: four features registered and lazily loaded, with TD-016 standing (`main.js` eagerly imported the clusters feature). ADR-048 replaced that mechanism entirely — the contract is now `FeatureRegistration` in `frontend/packages/kernel/src/feature/registration.ts` and the loader is `lazyFeature.ts`, with splitting done by Vite. It was returned to `REVIEW` because the audit that made it `COMPLETE` was of deleted code. **Completed 2026-09-06 on the exit condition this row named, and on the guard TD-016 was closed without.** `frontend/scripts/bundle-shape.mjs` reads `dist/.vite/manifest.json` after `pnpm -C frontend build` and refuses a bundle in which any `feature-*` chunk is reachable from the entry through static `imports`, or in which a feature has stopped being a dynamic import of any chunk. Run both ways on 2026-09-06: against the real manifest it printed `✓ 5 feature packages, all dynamically imported and none reachable statically from the entry chunk` and exited 0; against a copy with `packages/feature-topics/src/index.tsx` moved out of the entry's `dynamicImports` and into its `imports` it printed both violations and exited 1. It is a step of CI's `frontend` job, so the promise is now kept by a gate rather than by a convention. TD-022 is closed on the same evidence. |
...
| KU-016 | Smart-filter test execution is cluster-scoped and requires `TOPIC:MESSAGES_READ` | KUI-new | P1 | gateway | feature-messages | M3 | S | RESEARCHING | RBAC gap: Kafbat lets any authenticated user execute arbitrary CEL. Enforced from M6 when RBAC exists; the endpoint shape is fixed in M3. |
| KU-017 | Signed principal header gateway → services (request-bound, short expiry, per-service audience); services reject unsigned requests except in all-in-one mode | KUI-new | P0 | gateway, all services | — | M6 | M | COMPLETE | Audited 2026-09-04. Minted per call by the gateway, verified by every service, with inbound `X-Kui-*` stripped at the edge. As of 2026-09-04 it carries a real signed-in principal when a deployment configures `kui.auth.type`, and anonymous otherwise. |
| KU-018 | Pluggable server-side session store (in-memory default, shared store adapter) | KUI-new | P1 | gateway | — | M6 | M | REVIEW | Audited 2026-09-04. `InMemorySessionStore` is the only adapter (TD-003), so sessions are single-replica. Session fixation is now defended for real: signing in deletes the session the request arrived on and creates a new one, so the id and the CSRF secret both change (`SignInSeamSuite` asserts it over a socket). `rotate` itself remains uncalled — replacing the session outright is the stronger move and does not need it. |
| KU-019 | RBAC view: who can do what, per cluster and resource | KUI-new | P1 | gateway (read model) | feature-admin | M6 | M | RESEARCHING | Screen 30. Kafbat only exposes this through 403s. |
| KU-020 | Audit log viewer | KUI-new | P1 | identity | feature-admin | M6 | M | RESEARCHING | Screen 31. Reads the AD-001 sink. |
...
| KU-022 | Connector plugin config validation requires `CONNECTOR:CREATE` or `CONNECTOR:EDIT` | KUI-new | P1 | gateway | feature-connect | M7 | S | RESEARCHING | RBAC gap: Kafbat's validate endpoint has no permission check and no audit. |
| KU-023 | Extended built-in serdes: ProtobufFile, ProtobufRaw, AvroEmbedded, MessagePack, Struct, MirrorMaker2 heartbeat / offset-sync / checkpoint, `__consumer_offsets` | Kafbat | P1 | message | feature-messages (picker) | M5 | L | RESEARCHING | Split out of SD-001 so M3 stays bounded. Includes CG-009. |
...
| KU-029 | Event-tracking correlation-key grouping (`groups[]` in the track response) | KUI-new | P2 | message | feature-messages | M9 | M | RESEARCHING | Contract reserves the field in M3. |
...
| KU-033 | Fault-isolation E2E suite: for every service, stop its container and assert the shell, the other features and the fallback panels still work | KUI-new | P0 | e2e | — | M1 | M | REVIEW | Audited 2026-09-04, re-read 2026-09-06. **Corrected downward from COMPLETE.** The suite covered the cluster service only — 1 of 4 Kafka-facing services: `ClusterServiceDownSuite` was the only caller of the reusable `FaultIsolationScenario`. **The reason recorded here was wrong in two of its three parts.** `topic`, `message` and `consumer` each have a `Main.scala` under `services/<name>/app/` and each has a container image in `deployment/compose/docker-compose.yml`. Only *no client module* survives: `services/cluster/client` is the sole client module in the tree. **Re-read 2026-09-12, and the note above described code that no longer exists.** The Scala `e2e/` module it named was deleted at `41358502` when ADR-048 replaced the browser build — `git ls-files 'e2e/*'` answers nothing, and the only surviving mention of `FaultIsolationScenario` in this repository is in documentation. What does fault isolation today is `deployment/compose/smoke.sh`, which is a step of CI's `compose` job and therefore a gate, and which stops **one** container (`kui-cluster`) and asserts the gateway and the rest carry on and then recover. So the coverage figure this row records is unchanged at 1 of 4 and the mechanism behind it is different; *for every service* is still not met, and it is a script to extend rather than a suite to write. |
...
| DR-20 | Merge `kfeature-security-service` into `kui-cluster-service`? (D-8) | **Settled by ADR-004: no.** It has its own capability gate, its own failure signature and functionality that is not cluster topology; merging a slow optional feature into the one Core service would break fault isolation. Owner stays `security`. |
...
<!-- checked: milestones -- verified by ./scripts/feature-matrix-check.sh -- claims: milestone-rows, milestone-p0, milestone-p1, milestone-line, total-line, total-rows, total-split, total-p0, total-p1 -->
...
| M9 | 10 | 0 | 0 |
| — (rejected) | 4 | 0 | 0 |
| **Total** | **189** (150 from research + 39 KUI-new) | **74** | **72** |

<!-- /checked -->

States at seed time: 172 `RESEARCHING`, 7 `DEFERRED`, 4 `REJECTED`, 0 `DESIGNED`. ADR-042
(2026-09-03) rewrote OT-004 and added OT-007 … OT-010, and the table above counts 189 rows today.
...
table already agreed. **The M6 and total lines were recomputed again on 2026-09-06 (wave 4)**, for
the one row this file gained rather than for a drift: `KU-034`, the gateway's cross-entity search,
which shipped in wave 3 with no row anywhere in this table. Every other line was re-derived from the
rows and had not moved. The counts here are *recomputed*, never incremented, for the reason the
state totals at the top of this file record.
...
(delete, offset reset and offset deletion were all served and all refused a read-only cluster
before touching Kafka, with nothing in the interface calling them; all three are called from a
screen now, `CG-005` from wave 6) and `CG-006` (incremental lag polling answers
...
**A record of the audit pass at `debd783`, 2026-09-06, before wave 4 landed — a dated
measurement and not a live figure:** 188 rows, of which 82 `RESEARCHING`, 64 `COMPLETE`,
20 `REVIEW`, 8 `IMPLEMENTING`, 2 `PARTIAL`, 1 `SERVICE DONE, NO UI`, 7 `DEFERRED`, 4 `REJECTED`, and
no `BLOCKED` row. That pass built nothing; it read the tree against the rows and moved the four that
were wrong.

Read the date and the commit, not the numbers. These nine totals sit outside every
`<!-- checked: -->` marker and nothing compares them with the rows — the paragraph at the top of
this file carries the same nine *inside* one, and `./scripts/feature-matrix-check.sh` is what holds
those to the State column. Wave 4 then moved six rows and added one on the same calendar day, so
this paragraph and the checked one disagreed while carrying the same date, which is exactly how an
unmarked copy misleads: it is not wrong, it is undated. It now names the commit it was taken at.
Every later pass appends a record of its own here rather than editing this one; when a record and
the checked paragraph disagree, the checked paragraph is current and the record is history.

**A record of the wave-4 pass, 2026-09-06:** 189 rows, of which 80 `RESEARCHING`, 68 `COMPLETE`,
18 `REVIEW`, 9 `IMPLEMENTING`, 2 `PARTIAL`, 1 `SERVICE DONE, NO UI`, 7 `DEFERRED`, 4 `REJECTED`, and
no `BLOCKED` row. Six rows moved — `SR-001`, `SR-002`, `MT-002` and `KU-004` up, `CG-005` down, and
`BR-001`'s and `MT-001`'s notes corrected without a state change — and `KU-034` was added for a
capability that had shipped a wave earlier with no row.

**A record of the wave-6 pass, 2026-09-07:** 189 rows, of which 79 `RESEARCHING`, 70 `COMPLETE`,
18 `REVIEW`, 8 `IMPLEMENTING`, 2 `PARTIAL`, 1 `SERVICE DONE, NO UI`, 7 `DEFERRED`, 4 `REJECTED`, and
no `BLOCKED` row. Two rows moved, both up and both on a browser rather than on a report: `CG-005`,
whose forget control is on the shipped group detail page and is asserted there against the
coordinator's own topic list, and `CL-004`, whose five metrics cards draw against a stack built
...
load-bearing references rather than a backlog. `docs/plan/verification/W8-07.md` holds the
screen-to-case mapping this section refuses to duplicate, and the rule it took that mapping under —
a screen is counted covered *when a browser case asserts the content that capture is a picture of*
— is the rule the paragraph below restates. `docs/plan/verification/W8-04.md` holds the Connect
browser measurement the Connect list row's cell cites. `docs/plan/verification/W9-03.md` is cited
from inside `frontend/e2e/brokers.spec.ts` and `frontend/e2e/topics.spec.ts` as the record of what
those two cases used to fail to assert. **Those three stay until the file that cites them stops
citing them**, and deleting one without that is how a dangling reference is made — so this
paragraph is inside a checked region, where a backticked path that looks like a file in this
repository and is not one fails `./scripts/feature-matrix-check.sh` with the path printed, and
where the sentence quoted above is compared against the file it is attributed to. The two
citations from inside the browser specs are not in this file's ownership; moving them is a change
to the specs. Every other report in that directory is discharged and cited by nothing.
<!-- /checked -->
