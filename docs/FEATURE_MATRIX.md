...
| KU-004 | Feature registration contract and lazy feature loader (Vite code splitting, ADR-012 as amended by ADR-048) | KUI-new | P0 | — | kernel, shell | M0 | M | REVIEW | Audited 2026-09-04 against the Scala.js build: four features registered and lazily loaded, with TD-016 standing (`main.js` eagerly imported the clusters feature). ADR-048 replaced that mechanism entirely — the contract is now `FeatureRegistration` in `frontend/packages/kernel/src/feature/registration.ts` and the loader is `lazyFeature.ts`, with splitting done by Vite. Returned to `REVIEW` because the audit that made it `COMPLETE` was of deleted code: re-verify against a Vite build manifest, which is also TD-016's exit condition. |
