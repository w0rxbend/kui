...
| AU-005 | User menu: logout, theme auto/light/dark, timezone | Kafbat | P1 | — | shell | M1 | S | REVIEW | Audited 2026-09-04, **corrected downward from COMPLETE**, and partly recovered on 2026-09-05. The account menu now exists (`layout/UserMenu.scala`): it names the signed-in user, lists the roles they hold — the answer to "why can I not do that?" — and offers **Sign out**, which posts to `/auth/logout`, clears the principal, the CSRF token and every cached permission, and reloads; a failed request still clears and reloads. It is hidden entirely for an anonymous principal, so the no-login deployment is unchanged. Verified in a browser. The three-state theme control ships and now has an icon for its third state. **Still missing: the timezone and refresh-rate controls** (KU-012). |
