...
**Counts as of the 2026-09-05 verification pass** (188 capability rows): 63 `COMPLETE`, 13 `REVIEW`,
13 `IMPLEMENTING`, 3 `SERVICE DONE, NO UI`, 2 `PARTIAL`, 82 `RESEARCHING` (not started), 1 `BLOCKED`,
7 `DEFERRED`, 4 `REJECTED`. Excluding the 11 deferred and rejected rows, **63 of 177 in-scope
capabilities are delivered — 36%**, against 53 (30%) at the audit ten rows earlier.
...
| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AU-001 | Auth types: disabled, login form, OAuth2/OIDC (GitHub, GitLab, Google, Cognito, Azure Entra, generic), LDAP / AD | Kafbat, Provectus, Kouncil | P0 | identity (+gateway session) | shell | M6 | L | IMPLEMENTING | Backend delivered 2026-09-04. `disabled` (still the default), `form` and `oidc` are real: `kui.auth.type` is read, the identity service verifies passwords and completes an OpenID Connect flow, and the gateway serves `/api/v1/auth/{settings,login,password,oidc/start,oidc/callback}` with a session issued on success. LDAP and Active Directory are **not** implemented and are refused by name at start-up (ADR-015 Amendment 1); per-vendor group extractors are RB-002. The sign-in **screen** does not exist yet — the shell has nothing that calls these routes. |
| AU-002 | In-memory users with default accounts and forced password change | Kouncil | P2 | identity | shell | M6 | M | IMPLEMENTING | Delivered 2026-09-04, backend only. `kui.auth.users[]` holds a name, a PBKDF2 hash (`kui.identity.app.HashPassword` writes one), groups and `mustChangePassword`; the forced change is *enforced* — such an account gets a single-use challenge and no session until it has set a new password. The change is saved in the metadata store, and a deployment with none is told to configure `kui.store.dir` rather than losing the change silently. No screen calls it yet. |
...
| ID | Feature | Source | Priority | Owner | MFE | Milestone | Cx | State | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RB-001 | Role model: subjects × clusters × resource pattern × actions, with action dependencies | Kafbat, Provectus | P0 | identity (evaluation) + gateway (enforcement) | ui-admin (view) | M6 | XL | IMPLEMENTING | `kui.rbac.roles[]` and `kui.rbac.defaultRole` are read into `RbacPolicy` at start-up (2026-09-04), with an unknown action, an unnamed pattern on a named resource, an uncompilable regular expression and a duplicate role name all refused as configuration errors naming the key. Roles are resolved once at sign-in and travel in the session and the signed principal. Enforcement at the edge is a separate strand (`PolicyRbacPreCheck`); the screens that manage roles are RB-004. |
...
| RB-003 | Current user info and flattened permissions for UI gating | Kafbat, Provectus | P0 | gateway | kernel | M6 | S | IMPLEMENTING | `GET /auth/me` now answers with the real principal, its roles, and the expanded permission list (`Rbac.grants`), and `GET /auth/settings` says which sign-in this deployment uses. A deployment with no roles still answers a wildcard grant per resource rather than an empty list, so nothing is hidden in the quickstart. The browser half — gating write controls on that list — remains: `ActionPermissionWrapper` is still called from one site that passes no `permitted`. |
