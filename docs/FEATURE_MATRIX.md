...
**Counts as of the 2026-09-07 wave-5 recount** (189 capability rows): 68 `COMPLETE`, 18 `REVIEW`,
10 `IMPLEMENTING`, 1 `SERVICE DONE, NO UI`, 2 `PARTIAL`, 79 `RESEARCHING` (not started),
7 `DEFERRED`, 4 `REJECTED`, and no `BLOCKED` row. Excluding the 11 deferred and rejected rows,
**68 of 178 in-scope capabilities are delivered — 38%**.
<!-- /checked -->
...
repository calls its client function, so the row is `IMPLEMENTING`. And one row was **added**:
...
| CL-004 | Cluster-level aggregated metrics (JMX/Prometheus) | Kafbat, Provectus | P1 | metrics | feature-clusters | M8 | M | IMPLEMENTING | **Moved out of `RESEARCHING` 2026-09-07, and no further, on the evidence that exists.** Wave 5 added the four reads beside `throughput` — `…/metrics/latency`, `…/metrics/request-handlers`, `…/metrics/producers`, `…/metrics/record-size`, each `Section`-wrapped so one dead exporter costs one card (ADR-052) — routed them at the gateway, and built the cards that read them (`frontend/packages/shell/src/overview/`: `LatencyCard.tsx`, `TrafficCards.tsx`, `metrics.ts`). `RESEARCHING` in this file means nothing is built, and that is no longer true. What is missing is the only thing that makes a row `COMPLETE` here: **nobody has done it from a browser**. The quickstart running on this machine on 2026-09-07 was built from a tree older than these endpoints and answers `KUI-ROUTE-NOT-FOUND` on all four, so this pass could measure the cards against nothing; the browser cases exist — `frontend/e2e/traffic.spec.ts` gained five, one per card plus the stat row — and were not run here. Three of the design's figures are deliberately *not* drawn as drawn — purgatory is a queue length and not a percentage, top producers is by topic and not by `client.id`, and there is no record-size distribution — each decided in ADR-052 rather than derived and mislabelled. |
...
| CG-005 | Delete committed offsets for one topic | Kafbat | P1 | consumer | feature-consumers | M4 | S | IMPLEMENTING | Delivered 2026-09-04 (`05287d2`). Offered per topic, on the topics the group actually holds offsets on, with a receipt that names how many partitions were forgotten — so 'the group held none there' and 'they are gone now' stay different sentences. **Corrected downward 2026-09-06: the control is not in the shipped screen.** The endpoint ships and is refused on a read-only cluster, and the browser's client function ships — `deleteOffsets` in `frontend/packages/feature-consumers/src/write.ts:177` — but a repository-wide search for that name finds exactly one reference, its own declaration, and no file under `frontend/` contains the string *Forget this group's offsets on one topic* that the 2026-09-05 pass reported seeing on the group detail page. That pass was against the Scala.js screen ADR-048 deleted; the port to `feature-consumers` did not carry the control across and the row was not re-read. `IMPLEMENTING` is the state this file reserves for code that is built, tested and reachable from nothing, which is what the header calls **code-only**. Exit: the group detail page offers it per topic and the receipt is asserted at the route |
...
<!-- checked: milestones -- verified by ./scripts/feature-matrix-check.sh -->
