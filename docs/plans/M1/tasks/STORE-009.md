...
## Status: BLOCKED on CFGOP-004

Not implemented. The spec's own cross-area contract says so in as many words:

> If CFGOP-004 has not landed, this task is blocked (the DEVPLAN's edge says so). Do not fork a
> container fixture; two container definitions in one repository is how CI time doubles quietly.

At the time this lane reached STORE-009, `libs/testkit` contained no `kui.testkit.kafka` package,
no `KafkaContainers`, no `KafkaFixture` and no `KafkaTopology`; nothing in `build.mill` references
one. `libs/config/test` already carries the Testcontainers coordinates STORE-005 added, so the
only missing piece is the shared fixture itself, and forking it here is the one thing the spec
forbids.

**What is ready for it.** Every non-container prerequisite is in place and green:

- `libs.config.test` has `testcontainers-scala-munit`, `testcontainers-scala-kafka`,
  `kafka-clients` and `cats-effect-testkit` on its test classpath (STORE-005, STORE-006).
- `StoreTopics`, `StoreBootstrap`, `StoreClients`, `KafkaConfigStore`, `WriteWaiter`,
  `StoreHealthRef` and `FieldCrypto` are all implemented against the behaviour this suite asserts.
- The committed golden file `record-cluster-encrypted-k1.json` is generated by real encryption, so
  the canary-token shape the leak test needs already exists.

**What the container suite still has to prove**, and which no non-container test can:
`existingTopicWithCleanupPolicyDeleteFailsStartup` and its two siblings; every replay assertion
against a real log; the two-replica race and its convergence; read-your-writes through a separate
raw consumer; the raw-byte topic dump containing no canary; and the stop/start recovery. The
pure-Scala suites in this lane cover the *logic* of each of those — the version rule as a
property, the replay bound and both cancellation paths under simulated time, the required/advisory
classification, AES-GCM round-tripping and its AAD binding — but none of them is a substitute for
the broker-backed assertions, and this file should not be marked done until they exist.
