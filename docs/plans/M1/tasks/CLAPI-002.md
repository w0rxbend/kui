...
1. **The path and query codecs come from `KernelSchemas` rather than being declared in
   `ClusterEndpoints`.** They already exist there, with the decode-failure behaviour this spec
   asks for; declaring a second pair would be two definitions of one transport rule.
2. **`pingIsGone` is not in this task's suite**, per the gate review's amendment: `ping` is still
   declared here and CLAPI-004 deletes the family. The assertion exists, in CLAPI-004's commit,
   as `nothing named ping survives`.
3. **`ClusterEndpoints.ClusterIdParam` and `ConfigsSegment` added** to the named constants; the
   spec named `BrokerIdParam` and the segments but not these two, and a literal typed twice is
   what the constants exist to prevent.
4. `theEndpointListIsExactlyTheSixDeclared` is named for six because `ping` is gone by the time it
   is read; it was seven while `ping` was still declared.
