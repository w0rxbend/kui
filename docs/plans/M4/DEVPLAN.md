...
M4 is the milestone where KUI first changes a consumer group. It is **not** the product's first
mutation — M3 ships produce, resend and purge before it — so the safety substitutes below are not
M4's invention but an inherited rule, **ADR-047**, that M3 carries too. Everything else before M4
...
8. **Nothing crosses the process boundary twice.** `./mill checkArchitecture` passes with rule A14
   (§6.3) active: no consumer-group state name, reset-mode name or lag-anomaly name is declared
...
| --- | --- | --- | --- |
| P1 | `services/cluster/client`, the shared credential-bearing profile consumer. The question `ARCHITECTURE.md` §14 left open is now closed by **ADR-046**: credentials travel on `/internal/v1` and exactly one module consumes them. M2 TOP-008/TOP-009 build it | `test -d services/cluster/client` | GRP-022 builds that module — **in `services/cluster/client`, never a private copy inside `services/consumer/infrastructure`** — and its size rises from L to XL. Its spec carries both branches |
...
| P3 | `frontend/ui-topics` renders the `topic.tabs` guest host and `ui-kernel` declares `FeatureSlots` (KU-013, M2 TOP-027/TOP-030) | `grep -rn "topic.tabs" frontend --include='*.scala'` | GRP-035 ships `ui-consumers`' guest panel and its registration; `FeatureSlots` is added to `ui-kernel` by GRP-035 with the signature DC-H6 describes. `FeaturePanel` itself already exists from M0 |
| P4 | `libs/contracts-core`'s `PageDto` (M2 TOP-019) and `libs/kernel`'s `NameIndex` (M2 TOP-001), which the group list's paging (ADR-026) and its search (§10 D14) are both stated in terms of | `grep -rn "PageDto\|NameIndex" libs --include='*.scala'` | GRP-024 declares `PageDto` and GRP-014 the substring matcher, each with the shape M2's specs give, recorded in `TECH_DEBT.md` as a duplication for whichever milestone lands second to collapse |
...
| ADR-041 (+ Amendments 1–4) | A1, A3, A5, A9, A10 all apply unchanged to the new service tree; A11–A13 arrive with M2 and M3; M4 adds A14 (§6.3) |
| `libs/kafka/PORT-INVARIANTS.md` | **§2 (the fabricated dead group) is M4's and lands here** — GRP-002 moves it into `GroupAdmin.describeGroups`' scaladoc and deletes it from the file in the same commit. **§1 (the leaderless filter) is M2's**: TOP-005 implements it and deletes §1. M4 needs the same filter for lag, so GRP-006 **reuses M2's helper rather than writing a second one**; if M2 has not landed, GRP-006 creates it as the shared `OffsetLookup` that M2's `TopicAdmin.listOffsets` then calls — one implementation either way (§10 D8, gate review F-04). GRP-040 deletes the file when both sections are gone |
...
| GRP-040 | Rule A14, the milestone documentation, the matrix, and PORT-INVARIANTS' deletion | M | everything | F |
...
| R-6 | Six group-state names get typed a second time in a DTO, a query parameter or a CSS class | The M0 review's second finding, repeated | Rule A14 and its build test (§6.3); one cross-compiled declaration in `libs/kernel` that the domain, the port, the contract and the browser all compile against | GRP-001, GRP-040 |
...
3. `./mill __.compile` is clean with `-Werror -Wunused:all -source:future`; `./mill __.test` is
   ...
   with rule A14 active and has been proven to fail on a deliberate duplicate declaration.
...
9. `libs/kafka/PORT-INVARIANTS.md` §2 has been **moved into `GroupAdmin.describeGroups`' scaladoc
   and deleted from the file**. §1 belongs to M2; a `grep` for its rule finds it in exactly one
   place — whichever of `TopicAdmin.listOffsets` or `OffsetLookup` implements the filter — and
   never in two. The file is deleted entirely once both sections are gone.
...
11. ADR-006 records the group port in its consequences; ADR-016 and `ARCHITECTURE.md` §9 record the
    group snapshot's cache discipline; ADR-034 records the two new codes; **ADR-045** records the plan-token
   confirmation model of D3 and **ADR-047** the marker/refusal/audit trio of D2 — both written at
   the M2/M3/M4 gate rather than by GRP-040, because M3 ships mutations before M4 and inherits
   ADR-047 too. GRP-040 cites them and records what implementation learned.
