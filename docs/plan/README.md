...
5. When every packet's acceptance check passes, `WAVE-NN.md` is **deleted** and `WAVE-(NN+1).md` is
   written in the same commit. The deletion is the record that the wave closed; the git history
   holds what it said.
