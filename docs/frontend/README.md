...
Everything that runs in a browser. It is **TypeScript** with [SolidJS 2](https://www.solidjs.com/)
for the user interface, built by [Vite](https://vite.dev/), with plain CSS for the styling
(ADR-048, superseding ADR-011; ADR-012 and ADR-024 as amended).

It is its **own build**. `frontend/` is a pnpm workspace with its own `package.json`, its own
lockfile, its own tests and its own container image (`deployment/frontend/`). Mill does not build it,
the gateway's jar does not contain it, and the two halves talk over HTTP like any other client and
...
- **There is no virtual DOM and no re-render.** A Solid component function runs *once* and builds
  real DOM nodes. What changes afterwards is not the tree, it is the values flowing through the
  reactive bindings attached to it. A `props` access inside JSX is a live read, not a snapshot,
  which is why destructuring props breaks reactivity.
- **State is explicit and owned.** A signal is a value that can be read and written; a memo is a
  derived value that always has a current value; a store is a fine-grained reactive object. A
  component takes them from its caller and never creates global state of its own.

## Package layout

```
frontend/
  package.json          the workspace root: build, watch, test, typecheck, storybook
  pnpm-workspace.yaml
  vite.config.ts        the build; `base: "./"` so every asset URL is relative
  vitest.config.ts      the test runner; `environment: "jsdom"`
  index.html            the one page, with the two markers a deployment substitutes
  packages/
```
...
```
    api/                @kui/api — the contract seam, and nothing else
      src/
        schema.d.ts             generated from docs/api/openapi.browser.json — never edited
        constants.generated.ts  generated by `./mill frontend.apiConstants` — never edited
        client.ts               createApiClient: the one place a request is built
        errors.ts               ApiError and the four shapes a failure takes
        result.ts               ApiResult — ok / err, so nothing throws across the seam
        section.ts              the partial-response envelope and its status
        bootstrap.ts            readBootstrap: basePath, apiBase, buildVersion
        csrf.ts                 the CSRF token pair
        brand.ts                ClusterId, TopicName, GroupId — branded ids
```

```
    kernel/             @kui/kernel — the design system and the shell's plumbing
      src/
        components/     the primitives: Button, DataTable, Dialog, Drawer, StatCard, …
        data/           query/ (server state), sse/ (the two stream wrappers),
                        capabilities/, permissions/, fetched.ts, mutation.ts
        feature/        the KuiFeature registration contract and lazyFeature
        state/          the kernel-owned signals: session, currentCluster
        theme/          light / dark / follow the system
      styles/           NN-*.css — the stylesheet, in cascade order
```

```
    shell/              @kui/shell — the application: router, layout, its own pages
      src/
...
The package name and the directory agree — `@kui/feature-topics` lives in
`frontend/packages/feature-topics`. There is no Mill module for any of them.

## CSS

### One file per screen, concatenated in a fixed order

The stylesheets live beside the packages they belong to, at
`frontend/packages/<package>/styles/*.css`.

They are assembled into a single stylesheet in the order **tokens → reset → kernel → features**
(ADR-024 as amended by ADR-048 §6). Plain CSS has no import graph: when two rules match the same
element with equal specificity, the one written later wins, so the assembly order *is* the cascade,
and it is decided once rather than by whatever order the filesystem returns.

That order is written out as an explicit `@import` list in
`frontend/packages/kernel/styles/index.css`, which **Vite** inlines at build time. There is no Mill
task in this path any more; `kui.build.CssPipeline`, which used to scan the build for stylesheets
and sort them by role, is gone with the Scala.js build.

What a directory scan could not do is forget a file, and a hand-written list can. That property is
restored as a check: `kui.build.CssReferences` (in `build-tests/`, exercised by
`CssReferencesSuite`) fails the build if any `styles/*.css` file in the workspace is missing from
`index.css`, or named by it twice.

```bash
cd frontend && pnpm build       # Vite writes the stylesheet into frontend/dist/assets/
```
...
There is no preprocessor and no CSS-in-JS. Native CSS nesting and custom properties cover what Sass
was for, and a runtime style system was rejected for the reason ADR-024 gives: it moves work into
the browser that a build has already done, and it makes the cascade unreadable.

### Class names are written in the markup

```tsx
<div class="kui-card">…</div>
```

Under Scala.js every class name was reached through an object of constants (`KernelCss.Card`),
because a Scala string literal is unchecked and a typo in one was silent. TSX writes them directly:
the name sits next to the rule it selects, and the check that catches a typo is the stylesheet
itself and the component's own render test.

Naming is BEM with a `kui` prefix — `kui-<block>__<element>--<modifier>` — which keeps every
selector to a single class, so specificity is uniform and the cascade stays predictable.
...
If the stylesheet fails to load — a broken deploy, a proxy that drops it — the application must still be
...
`@kui/shell` is the **entry point**: `packages/shell/src/index.tsx` is the one module Vite is
pointed at, and everything else is reached from there. Feature packages are reached only through the
registry's dynamic imports — which is what lets Vite put each feature in a chunk the browser
downloads on demand (ADR-012 as amended by ADR-048 §4).
...
| --- | --- | --- |
| the route patterns | data — path shapes | eagerly, with the shell |
| the components | code | on the first navigation, through a dynamic `import()` behind Solid's `lazy()` |

Route patterns cost a few bytes in the entry chunk and pull nothing else in. The check that a real
component reference has not leaked across that line reads Vite's build manifest: it asks whether a
feature's module is in the entry chunk's `imports` (static) or its `dynamicImports` (split), which
is a fact about the emitted graph rather than a text search.
...
### A route matches a whole path, never a prefix

Without it a pattern matches a *prefix*: `/settings` would also claim `/settings/anything`, and the
404 for a mistyped sub-path would never appear. (The rule was written for Waypoint, which made the
prefix behaviour the default. It is kept because the failure it prevents — a mistyped sub-path
silently rendering the parent page — does not depend on the router.)
...
repository — it is reimplemented in SolidJS with KUI's own CSS and the kernel's tokens (ADR-024).
...
Two processes, because there are two builds. The gateway serves the API and — in development — the
built interface from one origin, so there is no proxy and no CORS to think about (ADR-012
amendment 1).

```bash
./mill devStart                # the whole product, in the background, on :8080
cd frontend && pnpm watch      # `vite build --watch`: rebuilds on every save
```

Edit a file, save it, refresh the browser. That is the entire loop. A rebuild takes about a second;
nothing restarts, and nothing is copied.

The first time, and after any dependency change, run `pnpm install` first. Without a
`frontend/dist` the gateway answers `/ui/` with a 503 and a page saying so — not a stack trace and
not an empty `200`.

### Why nothing is copied

`StaticRoutes` serves `/ui/…` from the classpath under `/web`. `./mill dev` and `./mill devStart`
put one extra directory at the front of that classpath, containing a symbolic link named `web`:

```
out/dev-assets/solid/web  ->  frontend/dist   (index.html, the hashed chunks, the stylesheet)
```

A classloader takes the first entry that has the file, so every asset — and `index.html` with it —
resolves through that link. Vite rewrites the files behind it in place, so the next request already
sees them. The comment on `dev` in `build.mill` is the authority on this.

### There is no hot module replacement

`vite build --watch` rebuilds the bundle; it does not swap modules into a running page. A refresh is
the loop. Anything that depends on in-page state — a form half filled in, a stream part way through
— starts again on refresh, so structure a page such that restarting is cheap rather than trying to
avoid it.

(Vite *can* run a dev server with HMR, and `vite.config.ts` records why KUI does not use one: the
gateway is the only server, and a second origin would mean a proxy, CORS and a `SameSite` question
in development that production does not have.)
...
invocations on the output directory: while `./mill dev` holds the terminal, every other `./mill`
command waits for it. `devStart` detaches, which releases that lock. Its log is in `out/dev.log`;
`./mill devStop` ends it. The `pnpm watch` in the other terminal is unaffected either way — it is a
different build and shares no lock.

## Tests

Frontend suites are [Vitest](https://vitest.dev/), run under jsdom (a `document` implemented in
JavaScript) rather than a real browser. jsdom is fast and good enough for structure, accessible
names, attributes and events; it is not a browser, and it approximates layout, so nothing here may
assert geometry — `vitest.config.ts` says so at the point where it is easiest to forget.

```bash
cd frontend
pnpm test                                  # every suite
pnpm test packages/kernel                  # one package's suites, by path
pnpm typecheck                             # tsc --build, which `vite build` does not do
```

jsdom is an ordinary workspace dependency and arrives with `pnpm install`. There is nothing to
install globally and nothing to place at the repository root — see
[`../development/toolchain.md`](../development/toolchain.md).

Mill runs none of this. `./mill __.test` is the backend's suites and needs no Node.

Storybook is the third surface: `pnpm storybook` serves every component's stories from fixtures with
no backend at all, and `deployment/storybook/` builds the same thing as a static image.
