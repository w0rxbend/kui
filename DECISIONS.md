...
| [ADR-054](docs/adr/ADR-054-connect-endpoints.md) | The connect service: the Kafka Connect REST API 2.3 and later is assumed with a per-connector fallback for older workers, a failure reason is the connector's own trace and not KUI's paraphrase, a restart is reported while it is still in flight, and no throughput is put on the wire because the Connect API does not measure one | Accepted | 2026-09-10 |
| [ADR-055](docs/adr/ADR-055-ksql-endpoints.md) | The ksql service: ksqlDB 0.14's `POST /ksql` and `POST /query` over HTTP/1.1 with `/query-stream` deliberately unused because it needs HTTP/2, the object list is one request over four `SHOW` statements so one pane cannot disagree with itself, and a push query is a stream that ends only when the caller or the server ends it | Accepted | 2026-09-11 |
| [ADR-056](docs/adr/ADR-056-ksql-result-region.md) | The ksqlDB result region: four distinguishable ways to have no rows, columns fixed by the first `phase` frame, a 500-row window that says what it dropped, rows that outlive the failure that ended them, and a destructive statement confirmed against the plan's text rather than the editor's | Accepted | 2026-09-11 |
| [ADR-057](docs/adr/ADR-057-design-captures-stay-outside-the-repository.md) | The design captures stay outside the repository: `.gitignore` stands, the committed artefact is the reading and not the pixels, every document that said *committed* is corrected, and definition-of-done item 1's subject becomes the `M01`…`M23` index rather than a working-tree directory | Superseded by ADR-058 | 2026-09-12 |
| [ADR-058](docs/adr/ADR-058-design-captures-are-tracked.md) | The design captures are tracked: `.gitignore` no longer ignores `screens/`, because the twenty-three captures are the denominator of definition-of-done item 1 and an exit criterion whose subject exists on one machine cannot be re-run by anyone who clones | Accepted | 2026-09-12 |
