...
| `kui-quickstart-kafka-connect` | `apache/kafka:4.3.1` | a Kafka Connect worker in distributed mode, so the Connect screens have a real worker to be honest about. **No connector is registered in it and none should be.** An empty list from a worker that answered is a measured empty list, and it is a different fact from "this deployment configured no Connect cluster" (the `not_configured` a quickstart without this container gives) and from a worker that will not answer. Planting a connector to make the screen look busy would erase exactly that distinction |
