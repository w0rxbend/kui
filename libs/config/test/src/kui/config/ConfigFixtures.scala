...
/** Turns a committed test fixture into a real file on disk.
  *
  * The loader takes `java.nio.file.Path`s, because that is what a process is given on its command line, but
  * the fixtures travel with the tests as classpath resources. Copying a resource into a temporary file is the
  * smallest bridge between the two, and it keeps the fixtures visible in the repository next to the suite
  * that reads them.
  */
object ConfigFixtures {
  ...
  /** An ad-hoc YAML document, for the cases where writing the file inline is clearer than adding another
    * committed fixture.
    */
