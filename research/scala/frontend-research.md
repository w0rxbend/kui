...
**Agent:** Research Agent F (Frontend)
**Date:** 2026-09-03
**Inputs:** `PLAN.md` §12 (frontend rows), §16, §21, §32, Appendix D
...
4. Which microfrontend loading option from PLAN §21 (A / B / C) is right for Scala.js 1.x + Mill, with evidence?
...
Nesting: `App` -> `ClusterPage` (`src/components/ClusterPage/ClusterPage.tsx:82-142`) mounts feature routes conditionally on `ClusterFeaturesEnum` flags computed at :46-72 (schema registry :95, connect :101-114, ksqlDB :120, ACL :126, dynamic config :132), default redirect to `brokers` (:138). This is exactly PLAN §16.5 "capability-gated navigation", implemented with per-cluster feature flags rather than a service capability registry. -> `Topics` (`src/components/Topics/Topics.tsx:18-30`) -> `Topic.tsx` renders a `Navbar role="navigation"` (:220-285) plus inner `<Routes>` (:291+).
...
| React context | Kernel-owned `Var`s: `AuthState`, `Capabilities`, `CurrentCluster`, `Notifications` (PLAN §21 allows exactly these) |
| `AbortController` | `FetchStream.get(url, _.abortStream(stream))` (README "FetchStream") or sttp `cancel` via `Future`+`AbortSignal` |

Feature-local state is a plain Scala class (`final class TopicsState(api: TopicsApi, cache: QueryCache)`) holding `Var`s and derived `Signal`s; no global singletons, matching PLAN §21 "No global mutable singletons except the kernel's documented `Var`s".
...
- Define a sealed `Page` ADT per feature; each feature contributes `List[Route[_ <: Page, _]]` (PLAN §21 `FeatureRoute`). The shell concatenates routes into one `Router[Page]` (`Router(routes, getPageTitle, serializePage, deserializePage, routeFallback, deserializeFallback)` — README "Router").
...
- Route fallback: `routeFallback = url => NotFoundPage(url)` and `deserializeFallback`. For PLAN §16.5, the shell wraps every feature route render in `capabilityGate(feature)(render)` which shows the `Unavailable(reason)` panel when the capability registry says so.
...
### 4. Microfrontend loading: evaluating PLAN §21 options A / B / C
...
- **Evidence:** Mill `ScalaJSModule.jsEnvConfig` variants; scala-js.org JS environments page; Kafbat e2e layout (`e2e-playwright/src/{features,steps,pages,support}`), PLAN §32.
