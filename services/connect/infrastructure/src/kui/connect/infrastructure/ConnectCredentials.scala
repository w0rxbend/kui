...
/** How each request to a Connect worker proves who KUI is.
  ...
  * every mechanism effectful keeps the call site identical for all three, so `ConnectHttp` contains no branch
  * on how it authenticates.
  *
  * ==Why this is not `RegistryCredentials`==
  *
  * It is the same three mechanisms and very nearly the same code, and that duplication is deliberate rather
  * than unnoticed. `RegistryCredentials` lives in `services/schema/infrastructure`, and ADR-041 rule A11
  * forbids one service reaching into another service's private layers — a `services/connect` that imported it
  * would be exactly the edge that rule exists to refuse. The shared type it should be built on is
  * `UpstreamAuthConfig`, which `libs/config` already declares for this purpose and whose own scaladoc says
  * the two enums "should become one type the next time the schema service is opened". When they do, this
  * class and that one collapse into one in `libs/http`. Until then there are two, and this is the one every
  * *new* HTTP dependency uses, so the count stops at two.
  *
  * A failure to *obtain* a credential is a `KuiError` rather than an exception, and it is deliberately
  * distinct from the worker refusing one: "KUI could not get a token from your identity provider" and "your
  * Connect cluster rejected KUI's token" send an operator to two different systems.
  */
...
object ConnectCredentials {
  ...
  /** How long before a token's stated expiry KUI fetches a new one.
    ...
    * more than a Connect call's own budget, so a token handed to a request outlives that request.
    */
  ...
  /** Nothing to prove, which is what a Connect cluster on a private network ordinarily wants. */
  ...
  /** OAuth 2.0 client credentials, with the token cached until shortly before it expires.
    *
    * @param backend
    *   the transport for the *token endpoint*, which must not be the worker's own resilient backend: that one
    *   fails over between the Connect cluster's addresses, and a token request sent to a worker because the
    *   issuer was briefly slow would be a request for a password sent to the wrong system.
    */
  def oauth[F[_]: Async](
  ...
  /** The credentials one Connect cluster's configuration asks for.
    *
    * A `Resource` because the OAuth case holds a cached token for the life of the process.
    *
    * @param tokenBackend
    *   the transport for the token endpoint, which the caller builds as its own upstream — see
    *   `ConnectWiring` for why it must not be the worker's. `None` is legal for the two mechanisms that need
    *   no issuer; an OAuth configuration with no backend is a wiring mistake, and it produces credentials
    *   that refuse every call with that sentence rather than a `NoSuchElementException` at the first request.
    */
  ...
  /** Credentials that cannot be obtained, as a value.
    *
    * It fails the request rather than the process: a Connect cluster KUI cannot authenticate to must show one
    * unavailable row, exactly like a Connect cluster that is down, and never stop a service that is also
    * serving three other clusters.
    *
    * `Remote` rather than `Unreachable`, and the difference is the sentence an operator reads.
    * `InfrastructureError.Unreachable` renders as *"kafka-connect-oauth could not be reached"* and keeps its
    * `cause` for the log only — deliberately, because a connection failure's text routinely contains a URL
    * with a password in it. Here there is no connection and no failure text: the cause is KUI's own wiring,
    * it contains nothing of the operator's, and it is the only thing that tells them where to look. A row
    * saying the issuer could not be reached would send them to an identity provider that was never called.
    */
  ...
  final private class OAuthCredentials[F[_]: Async](
    ...
    /** The cached token while it is still comfortably valid, otherwise a fresh one.
      ...
      * identity provider that has stopped answering would block every Connect request behind it — turning a
    ...
    private def fetch(now: Instant): F[Either[KuiError, CachedToken]] = {
      ...
      basicRequest
        ...
        .flatTap {
          case Left(error) =>
            logger.warn(
              s"the Kafka Connect OAuth token could not be obtained from ${config.tokenEndpoint.value}: " +
                error.message
            )
