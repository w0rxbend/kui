...
/** A `CapabilityRegistry` that records instead of deciding.
  ...
  * components — the readiness poller, the proxy routes, the capability endpoints — need to assert what was
  * reported to the registry, and would otherwise have to reason about a ten-second debounce window that has
  ...
  * is a gateway type, ADR-041 rule A5 forbids a library module from depending on a service module, and
  * `./mill checkArchitecture` enforces it. Putting the fake in `libs/testkit` would mean either moving the
  * registry out of the gateway — which ADR-041 §1a explicitly decided against — or breaking the check. It
  * lives in the gateway's own test module instead, and `services.gateway.api.test` depends on that module so
  * the two suites that need it can share one fake.
  */
