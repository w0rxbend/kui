...
/** Configuration to serdes: the one place that turns what an operator wrote into what decodes a record.
  ...
  * decides which serde reads which topic and was only ever handed `Rules.empty`; `SchemaRegistrySerdeFactory`
  * builds a registry-backed serde and was constructed nowhere, so every Avro payload in the product rendered
  * as bytes. The gap between them was two small translations, and this is both of them — which is why it
  * lives in the composition root, the one module allowed to see `libs/config` and `libs/serde-confluent` at
  * the same time.
  *
  ...
  * other than a YAML file, so it cannot name `libs/config`'s. Two small records and one function between them
  * is the cost of keeping both of those true, and the function is nine lines.
  */
object ClusterSerdeFactories {
  ...
  /** The serde factories one cluster has: the registry serde when a registry is configured, nothing when it
    * is not.
    *
    * A cluster with no `schemaRegistry` block gets an empty list, and that is a complete answer rather than a
    * degraded one — the built-in serdes and the fallback are what `ClusterSerdes` adds to whatever this
  ...
  /** Credentials, with the one case the decoder cannot honour named rather than silently downgraded.
    ...
    * with the registry's own refusal. Quietly sending nothing and calling it anonymous would produce the same
    * 401 with no explanation anywhere.
    */
  ...
  /** The operator's `serde` section as the resolution table's `Rules`.
    ...
    * One configuration entry can carry a key pattern and a value pattern, and it becomes one rule per target
    * here because resolution asks about one target at a time.
    */
  ...
  /** Whether any configured cluster needs an HTTP client at all.
    *
    * The message service opens no connection pool when no cluster has a registry, which is the ordinary case:
    * a process that holds an idle HTTP client and a set of otel instruments for an upstream nobody configured
    * is a process whose dashboards carry a permanently zero series.
    */
