...
/** Writing this service's OpenAPI document to a file, and checking that the committed one is current.
  ...
  * every service's document into one (ADR-003), and a merge that had to build nine services first would be a
  * merge nobody could run.
  *
  * A committed generated file is only trustworthy if something fails when it goes stale, which is what
  * `--check` is for: `./mill services.alerts.api.openApiCheck` fails when the file on disk no longer matches
...
object OpenApiDocument {
  ...
  /** The document, as the exact text that belongs on disk: two-space indented JSON with a trailing newline,
    ...
    */
  def render: String = AlertsApi.openApi[cats.Id].asJson.deepDropNullValues.spaces2 + "\n"
  ...
  def main(args: Array[String]): Unit = {
    ...
    if !check then {
      ...
      Console.err.println(s"$path does not exist; run ./mill services.alerts.api.openApi")
      sys.exit(1)
    } else if java.nio.file.Files.readString(file) != expected then {
      Console.err.println(
        s"$path is out of date with the endpoints it is generated from; " +
          "run ./mill services.alerts.api.openApi and commit the result"
      )
      sys.exit(1)
    }
  }
