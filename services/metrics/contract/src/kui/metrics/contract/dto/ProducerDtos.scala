...
/** The busiest topics, largest rate first.
  ...
  *
  * `internalTopicsExcluded` is the other field whose whole job is to keep the card checkable. Kafka's own
  * `__`-prefixed topics are taken out of the ranking before it is cut to size — on the quickstart broker
  * `__consumer_offsets` carries three orders of magnitude more bytes than any application topic, so a card
  * that ranked it would report the consumer group protocol as the busiest producer — and a list that quietly
  * omitted rows would be one nobody could reconcile against the exporter. The count travels so the browser
  * can say what was left out instead of the server deciding silently (ADR-052).
  *
  ...
  */
final case class TopProducersDto(
    measuredBy: String,
    topics: List[TopicProducerDto],
    internalTopicsExcluded: Int
)
  ...
  given Codec[TopProducersDto] = Codec.from(
    (cursor: HCursor) =>
      for {
        ...
        excluded <- cursor.getOrElse[Int]("internalTopicsExcluded")(0)
      } yield TopProducersDto(measuredBy, topics, excluded),
    (producers: TopProducersDto) =>
      Json.obj(
        "measuredBy" -> producers.measuredBy.asJson,
        "topics" -> producers.topics.asJson,
        "internalTopicsExcluded" -> producers.internalTopicsExcluded.asJson
      )
  )
  ...
  given TapirSchema[TopProducersDto] = TapirSchema
    ...
    .description(
      "The busiest topics by bytes in, largest first. `measuredBy` is `topic`: a broker publishes no " +
        "per-client.id byte rate unless quotas are configured (ADR-052). Kafka's own `__` topics are not " +
        "ranked, and `internalTopicsExcluded` says how many were left out"
    )
