...
/** Everything `kui-metrics-service` serves, and the one place a typed failure becomes an HTTP response.
  *
  * It is the schema service's `api` module with a different set of routes, deliberately: the five things an
  ...
  *
  *   - it binds the endpoints `services/metrics/contract` publishes to the use cases in
  *     `services/metrics/application`, with no path written out here (ADR-003);
  ...
  *   - it maps application types to contract types ([[MetricsMapping]], ADR-033);
  *   - it starts nothing. `services/metrics/app` is the only module in this service allowed to.
  */
object MetricsApi {

  /** The process's name, as it appears in `service.name` on every log line, span and metric. */
  val ServiceName: String = "kui-metrics"

  /** This service's identity in a signed principal's `aud` claim and in a capability document.
    *
    * It has to equal the id the gateway is configured with — `kui.gateway.services.metrics` — or every call
    * from the gateway is refused with a 401 that names no cause the caller can see.
    */
  val Id: ServiceId = ServiceId.unsafe("metrics")
  ...
  val Title: String = "KUI Metrics service"
