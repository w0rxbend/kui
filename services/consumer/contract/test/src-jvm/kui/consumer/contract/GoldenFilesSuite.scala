...
/** That every golden constant is exactly the file committed beside it.
  ...
  * duplicated document is only safe if something checks the two copies against each other. This is that
  * check, and it is JVM-only because it is the half that touches a filesystem.
  */
