...
/** The staleness contract: how "how fresh is this?" becomes a section of a response.
  ...
  * person — a known gap that is still open there. `SnapshotStatus.Offline` carries the `KuiError` itself, so
  * [[reasonOf]] can classify it, and this code does not repeat the mistake.
  *
