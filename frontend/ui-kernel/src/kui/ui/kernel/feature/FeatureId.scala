...
enum FeatureId(val value: String, val serviceId: String) {
  ...
  /** Consumer groups. The feature is `consumers` and the service behind it is `consumer`, singular — the same
    * near-miss `Topics` has, and the same reason neither is guessed from the other.
    */
