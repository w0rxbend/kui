...
/** Keeps `Tokens.scala` and `10-tokens.css` from drifting apart.
  *
  * Scala cannot see a CSS file, so nothing but this test stops the two lists diverging: a token
  * added to the stylesheet and forgotten in `Tokens` is invisible to every later reader, and a
  * constant in `Tokens` naming a property nothing defines resolves to an empty string at run time
...
final class TokensSuite extends FunSuite {
  ...
  test("the set is around forty tokens, not hundreds") {
    ...
    assert(Tokens.all.size <= 60, s"${Tokens.all.size} tokens; see docs/frontend/tokens.md")
  }
...
private object TokensSuite {
  ...
  /** The stylesheet with its comments stripped, so that a token name merely *mentioned* in prose is
