...
final class TokensSuite extends FunSuite {
  ...
  test("the set is dozens of tokens, not hundreds") {
    ...
    //
    // The ceiling was 60, then 80, then 88, and is 89. It moved the first time when the design
    // import (UI-013) brought a five-deep surface ramp, a paired text colour for every container,
    // and a second and third radius step. It moved the second time when the design screenshots were
    // read in full and turned out to draw five things the palette could not name: two more steps of
    // the text ramp (`text-strong`, `text-subtle`), five chart series, a card border that differs
    // by theme, the brand gradient, and the three fixed measurements of the application frame. It
    // moved the third time for a single token, `--kui-color-series-6`: the storage card draws four
    // segments inside one bar, and with only five series colours — three of which carry status
    // meaning — a fourth segment would have had to repeat an ink already used in the same mark.
    //
    // All three moves are the palette getting *more complete*, which is the opposite of the failure
    // this guard exists to catch. The first five series tokens in particular add no colour at all —
    // each is an alias of ink that already existed. Move it again only for the same kind of reason,
    // and write down what the reason was.
    assert(Tokens.all.size <= 89, s"${Tokens.all.size} tokens; see docs/frontend/tokens.md")
  }
