...
/** The two files the design-system tests read, found on disk rather than compiled in.
  ...
  * generated Scala source file that was compiled into the test binary. That machinery is gone: these suites
  * are ordinary JVM tests now (ADR-048 §5), and a JVM test can simply open the files. The inputs are the same
  * two files, and they are still the shipped ones — not copies, not fixtures.
  *
  * ## Finding the repository root without being told where it is
  *
  * A test does not get to choose its working directory, and the answer differs between running the whole
  * build, running one module, and running a single test from an editor. Rather than depend on any of that,
  * this walks up from the working directory until it finds the directory that contains `build.mill`, which is
  * the repository root by definition. If it reaches the filesystem root without finding one, it fails loudly
  * and says what it was looking for — a wrong path here would otherwise surface as an unreadable file several
  * frames away.
  */
object DesignSources {
  ...
  /** The token documentation, which is where the checked contrast pairs are listed.
    *
    * Reading the pair list out of the documentation rather than out of a test file is deliberate and is the
    * reason this file is an input at all: adding a row to the table there makes the suite check that pair, so
    * the documentation cannot quietly stop describing what is enforced.
    */
