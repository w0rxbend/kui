...
/** The build-level test UI-001 asks for: the concatenation order is the one ADR-024 decided, and the output
...
final class CssPipelineSuite extends FunSuite {
  ...
  test("orders tokens, then reset, then kernel, then features") {
    ...
    assertEquals(
      ordered.map(_.fileName),
      List("10-tokens.css", "00-reset.css", "20-kernel-controls.css", "00-clusters.css")
    )
  }
  ...
  test("files inside one group are ordered by module then by file name") {
    ...
    assertEquals(
      ordered.map(_.fileName),
      List("20-kernel-controls.css", "21-kernel-overlays.css", "50-later.css")
    )
  }
